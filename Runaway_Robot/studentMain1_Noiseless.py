...
def estimate_next_pos(measurement, OTHER = None):
	...
	if len(OTHER) > 1:
		for i in range(len(OTHER) - 1):
			steps.append(distance_between(OTHER[i+1], OTHER[i]))
			headings.append(atan2(OTHER[i+1][1] - OTHER[i][1], OTHER[i+1][0] - OTHER[i][0]))
		
		average_step = sum(steps) / float(len(steps))
	
	# I assume a constant turning angel
	turnings = []
	if len(headings) > 1:
		for i in range(len(headings) - 1):
			turnings.append( (headings[i+1] - headings[i]) % (2*pi))
		average_turning = sum(turnings) / float(len(turnings))
	else:
		headings.append(0.0)
		
	# x = x + cos(theta + diff_angle)
	# y = y + sin(theta + diff_angle)
	xy_estimate = (measurement[0] + average_step * cos(headings[len(headings) - 1] + average_turning),  \
	measurement[1] + average_step * sin(headings[len(headings) - 1] + average_turning) ) 

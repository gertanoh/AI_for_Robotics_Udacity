...
class robot:
    ...
    #
    def move(self, motion, beta_tolerance=0.001): # Do not change the name of this function
    
        distance1 = motion[1]
        steering1 = motion[0]
        # Apply noise
        distance = random.gauss(distance1, self.distance_noise)
        steering = random.gauss(steering1, self.steering_noise)
    ...
    def sense(self, add_noise=1): 
        ...
        for i in range(len(landmarks)):
            diff_y = landmarks[i][0] - self.y 
            diff_x = landmarks[i][1] - self.x  
            bearing = atan2(diff_y, diff_x) - self.orientation

...
def slam(data, N, num_landmarks, motion_noise, measurement_noise, world_size=100.0, plot=False):
    
    m_size = N + num_landmarks # i.e. 3 pos and 2 landmarks, matrix is 5*5
    all_mu = []
    dimen = 2
    ...
    for k in range(dimen):
        ...
        for i in range(N - 1):
            ...
            # update omega matrix  and Xi for each motion point
            ...
            for landmark in data[i][0]:							
                ...
                Omega[N + landmark[0]][N + landmark[0]] += 1 / measurement_noise
                
                Omega[i][N + landmark[0]] -= 1 / measurement_noise
                Omega[N + landmark[0]][i] -= 1 / measurement_noise
                
                Xi[i][0] -= landmark[k+1] / measurement_noise
                Xi[N + landmark[0]][0] += landmark[k+1] / measurement_noise
    ...
    if plot:
        plt.scatter(all_mu[0][N:], all_mu[1][N:], label='Estimated landmarks positions', c='g')
        plt.plot(all_mu[0][:N], all_mu[1][:N], label='Estimated robot path', c='g')

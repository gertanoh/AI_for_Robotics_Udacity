...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    m_size = 1 + num_landmarks # i.e. only keep the current position and the map
    all_mu = []
    dimen = 2
    ...
    # Apply slam for the dimension
    for k in range(dimen):
        Omega = [[0.0 for row in range(m_size)] for col in range(m_size)]
        Xi = [[0.0] for row in range(m_size)]
        ...
        # classical slam for point Xt
        for i in range(N - 1):                   
            # update omega matrix and Xi matrix for landmark
            # landmark is of type [index, dx, dy]
            for landmark in data[i][0]:							
                Omega[0][0] += 1.0 / measurement_noise
                Omega[1 + landmark[0]][1 + landmark[0]] += 1.0 / measurement_noise
                
                Omega[0][1 + landmark[0]] -= 1.0 / measurement_noise
                Omega[1 + landmark[0]][0] -= 1.0 / measurement_noise
                
                Xi[0][0] -= landmark[k+1] / measurement_noise
                Xi[1 + landmark[0]][0] += landmark[k+1] / measurement_noise
                
            
            Omega_m = matrix(Omega)
            
            Xi_m = matrix(Xi)
            
            row_list = [0] + range(2, Omega_m.dimx + 1)
            col_list = [0] + range(2, Omega_m.dimy + 1)
            Xi_list = [0] + range(2, Xi_m.dimx + 1)
            # expand matrix
            Omega_m = Omega_m.expand(Omega_m.dimx + 1, Omega_m.dimy + 1, row_list, col_list)
            
            
            Xi_m = Xi_m.expand(Xi_m.dimx + 1, Xi_m.dimy, Xi_list, [0])
            
            
            # update motion
            
            Omega_m.value[0][0] += 1.0 / motion_noise
            Omega_m.value[1][1] += 1.0 / motion_noise
            Omega_m.value[0][1] -= 1.0 / motion_noise
            Omega_m.value[1][0] -= 1.0 / motion_noise
            
            Xi_m.value[0][0] -= data[i][1][k] / motion_noise
            Xi_m.value[1][0] += data[i][1][k] / motion_noise
            
            
            # Use formula 
            # take to extract matrices
            A = Omega_m.take([0], range(1, Omega_m.dimy) )
            
            B = Omega_m.take([0], [0])
            
            C = Xi_m.take([0],[0])
            
            Omega_extract = Omega_m.take(range(1, Omega_m.dimx), range(1, Omega_m.dimy))
            
            Xi_extract = Xi_m.take(range(1, Xi_m.dimx), [0])
            
            
            Omega_m = Omega_extract - ( A.transpose() * B.inverse() * A)
            Xi_m = Xi_extract - ( A.transpose() * B.inverse() * C)
            
            Omega = Omega_m.value
            Xi = Xi_m.value
            
        # A dimension is done
        final_Omega_values.append(Omega)
        mu_tmp = matrix(Omega).inverse() * matrix(Xi)		
        all_mu.append(mu_tmp.value)
        
    
    # Format all_mu to return mu
    mu = matrix([c for point in zip(*all_mu) for c in point])
    
    # format Omega for return
    Omega = []
    # Add X values interpolated with y values for the grader
    for i in range(1+num_landmarks):
        x = final_Omega_values[0][i]
        y = final_Omega_values[1][i]
        row_x = []
        row_y = []
        for j in range(1+num_landmarks):
            row_x.append(x[j])
            row_x.append(0)
            row_y.append(0)
            row_y.append(y[j])
